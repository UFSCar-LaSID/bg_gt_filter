...
Official repository for "How Much Context Do 3D Object Detectors Need? An Oracle-Based Sensitivity Analysis".

## Abstract

Modern LiDAR-based 3D object detection operates on increasingly dense point clouds, where background regions dominate the data, imposing a massive, often redundant computational burden. Our spatial analysis of the nuScenes dataset reveals that over 90% of points belong to background classes, which typically contribute zero value to bounding box localization.  By leveraging pointwise semantic ground-truth annotations, we isolate the intrinsic impact of spatial context on deep perception stacks. We subject six distinct pillar- and voxel-based architectures to a clean-slate evaluation in fully filtered environments, alongside a controlled analysis of spatial context scaling to measure background dependence. Our empirical findings uncover a sharp architectural asymmetry: while pillar-based frameworks achieve the highest absolute gains in detection accuracy (up to +28.8% mAP), voxel-based detectors exploit spatial sparsity to secure dramatic computational savings, slashing complexity by up to 50.7% GFLOPS. Furthermore, we track end-to-end carbon footprints, demonstrating an 81.5% drop in estimated CO₂ emissions for localized inference. This systematic exploration maps the trade-offs among geometric detail, localized context, and system latency, providing foundational insights for designing next-generation 3D object detectors.
