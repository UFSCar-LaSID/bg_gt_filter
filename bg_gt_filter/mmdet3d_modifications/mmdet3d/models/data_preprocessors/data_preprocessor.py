...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self,
        ...
        if voxel:
            if 'filter_bg' in voxel_layer:
                self.filter_bg = voxel_layer.pop('filter_bg')
            else:
                self.filter_bg = False
            
            if 'remove_seg' in voxel_layer:
                self.remove_seg = voxel_layer.pop('remove_seg')
            else:
                self.remove_seg = True
    ...
    def voxelize(self, points: List[Tensor],
        ...
        # print('data_samples')
        # print(data_samples)
        # print(data_samples.gt_pts_seg.pts_semantic_mask)
        # quit()
        # print(data_samples)
        ...
        if self.voxel_type == 'hard':
            ...
            for i, res in enumerate(points):
                if self.filter_bg and data_samples and data_samples[i].eval_ann_info is not None and 'pts_semantic_mask' in data_samples[i].eval_ann_info:
                    res = torch.hstack((res, torch.from_numpy(data_samples[i].eval_ann_info['pts_semantic_mask'].reshape(-1, 1)).cuda()))
                elif self.filter_bg and data_samples and data_samples[i].gt_pts_seg is not None and 'pts_semantic_mask' in data_samples[i].gt_pts_seg:
                    res = torch.hstack((res, data_samples[i].gt_pts_seg.pts_semantic_mask.reshape(-1, 1)))
                ...
                #if self.filter_bg:
                #    bg_scores = 1 - res_voxels[:, :, -1].sum(dim=1) / res_num_points
                #    filter_mask = bg_scores < 1 # 1
                #    if filter_mask.sum() == 0:
                #        filter_mask[0] = True  # keep at least one voxel
                #    res_voxels = res_voxels[:, :, :-1]
                #    res_voxels = res_voxels[filter_mask]
                #    res_coors = res_coors[filter_mask]
                #    res_num_points = res_num_points[filter_mask]
                #    res_voxel_centers = res_voxel_centers[filter_mask]
            ...
            if self.filter_bg:
                bg_scores = 1 - voxels[:, :, -1].sum(dim=1) / num_points
                filter_mask = bg_scores < 1 # 1
                if filter_mask.sum() == 0:
                    filter_mask[0] = True  # keep at least one voxel
                
                voxels = voxels[filter_mask]
                if self.remove_seg:
                    voxels = voxels[:, :, :-1]  # Remove os labels dos pontos no final
                coors = coors[filter_mask]
                num_points = num_points[filter_mask]
                voxel_centers = voxel_centers[filter_mask]
