...
class _Voxelization(Function):
    ...
    def forward(
        ...
        if max_points == -1 or max_voxels == -1:
            ...
            # if all points are outside the range, we need to create one point to avoid empty output.
            all_points_outside_range = (points[:, :3] < torch.tensor(coors_range[:3], device=points.device)).any(dim=1) | (points[:, :3] >= torch.tensor(coors_range[3:], device=points.device)).any(dim=1)
            if all_points_outside_range.all():
                points = points.new_zeros(size=(1, points.size(1)))
