...
class LoadPointsFromMultiSweeps(BaseTransform):
    ...
    def __init__(self,
                 ...
                 add_sweep_idx: bool = False,
                 use_all_sweeps: bool = False,
        ...
        self.add_sweep_idx = add_sweep_idx
        self.use_all_sweeps = use_all_sweeps
        self.sample_size = sample_size

        if self.load_seg:
            self.use_dim += [max(self.use_dim) + 1]
        
        if self.add_sweep_idx:
            self.use_dim += [max(self.use_dim) + 1]
    ...
    def _load_points_labels(self, filename: str) -> np.ndarray:
        try:
            mask_bytes = get(
            ...
            mmengine.check_file_exist(filename)
            pts_semantic_mask = np.fromfile(
                filename, dtype=np.uint8)
    ...
    def transform(self, results: dict) -> dict:
        ...
        points.tensor[:, 4] = 0
        if self.load_seg:
            labels = self._load_points_labels(results["pts_semantic_mask_path"])
            points.tensor = torch.hstack((points.tensor, points.tensor.new_tensor(labels.reshape(-1, 1))))
        if self.add_sweep_idx:
            points.tensor = torch.hstack((points.tensor, points.tensor.new_zeros((points.tensor.shape[0], 1))))
        sweep_points_list = [points]
        if self.load_seg:
            sweep_points_list[0].points_dim += 1
        if self.add_sweep_idx:
            sweep_points_list[0].points_dim += 1
        ...
        if 'lidar_sweeps' not in results:
            ...
            for idx in choices:
                ...
                if self.load_seg:
                    labels = self._load_points_labels(f'/mmdetection3d/data/nuscenes/lidarseg_bgfg/v1.0-trainval/{sweep["sample_data_token"]}_lidarseg.bin')
                    points_sweep = np.hstack((points_sweep, labels.reshape(-1, 1)))
                if self.add_sweep_idx:
                    points_sweep = np.hstack((points_sweep, np.full((points_sweep.shape[0], 1), idx + 1)))
                if self.remove_close:
        ...
        if self.sample_size is not None:
            points = points[int(len(points) * self.sample_size):]
...
class LoadAnnotations3D(LoadAnnotations):
    ...
    def _load_masks_3d(self, results: dict) -> dict:
        ...
        try:
            ...
            pts_instance_mask = np.frombuffer(mask_bytes, dtype=np.uint64)
        except ConnectionError:
