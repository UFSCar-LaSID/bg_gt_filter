...
class ObjectSample(BaseTransform):
    ...
    def remove_points_in_boxes(points: BasePoints,
                               pts_semantic_mask: Optional[np.ndarray],
        ...
        if pts_semantic_mask is not None:
            pts_semantic_mask = pts_semantic_mask[masks]
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if sampled_dict is not None:
            ...
            points, pts_semantic_mask = self.remove_points_in_boxes(points, pts_semantic_mask, sampled_gt_bboxes_3d)
            if points.shape[1] != sampled_points.shape[1]:
                sampled_points = LiDARPoints(torch.hstack([sampled_points.tensor, torch.ones((sampled_points.shape[0]), dtype=sampled_points.tensor.dtype, device=sampled_points.tensor.device).reshape(-1, 1)]), sampled_points.points_dim + 1, sampled_points.attribute_dims)
        ...
        if pts_semantic_mask is not None:
            input_dict['pts_semantic_mask'] = np.concatenate([np.ones(sampled_points.shape[0]), pts_semantic_mask])
